...
pub struct ComplexFuncTransform<Trans, M>
where
    Trans: Fn(&Complex<f32>) -> Complex<f32> + 'static,
    M: Transformable + 'static,
    M::Intermediate: Interpolatable + InstanceUpdater,
{
    /// The unique identifier of the mesh to animate.
...
impl<Trans, M> ComplexFuncTransform<Trans, M>
where
    Trans: Fn(&Complex<f32>) -> Complex<f32> + 'static,
    M: Transformable + 'static,
    M::Intermediate: Interpolatable + InstanceUpdater,
{
    pub fn new(mesh_handle: &MeshHandle<M>, transform: Trans) -> Self {
        Self {
            mesh_id: mesh_handle.identifier(),
            transform,
...
impl<Trans, M> Animation<'static> for ComplexFuncTransform<Trans, M>
where
    Trans: Fn(&Complex<f32>) -> Complex<f32> + 'static,
    M: Transformable + 'static,
    M::Intermediate: Interpolatable + InstanceUpdater,
            ...
            move || {
                ...
                *transformed.borrow_mut() = Some(
                    mesh_pool
                        ...
                        .acquire_mesh_unchecked::<M>(self.mesh_id)
                        .apply_transform(|point: &[f32; 3]| {
                            let transformed = (self.transform)(&Complex::new(point[0], point[1]));
                            return [transformed.re, transformed.im, point[2]];
                        }),
                );
            }
        });
